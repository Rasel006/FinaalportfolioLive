const loadblog = () => {
    fetch("https://mdraselportfolioapi.onrender.com/blog/list/")
      .then((res) => res.json())
  ...
  const displaySingleBlog = (single_data) => {
    ...
    div.innerHTML = `
      <h1>Title: ${single_data.Title}</h1>
      <h2><img src="Profile.png" class="blog-image-design"> ${single_data.Publisher_name} · October 26, 2023</h2>
  ...
  const loadSingleBlog = () => {
    ...
    if (blogId) {
      fetch(`https://mdraselportfolioapi.onrender.com/blog/list/${blogId}/`)
        .then((res) => res.json())
